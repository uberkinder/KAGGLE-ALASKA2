...
class CsvLogger(LightningLoggerBase):
    ...
    def __init__(self, trn_csv_path, val_csv_path, trn_columns, val_columns):
        super().__init__()

        self.trn_csv_path = trn_csv_path
        self.val_csv_path = val_csv_path

        Path(self.trn_csv_path.parent).mkdir(exist_ok=True, parents=True)
        Path(self.val_csv_path.parent).mkdir(exist_ok=True, parents=True)

        self.trn_columns = trn_columns
        self.val_columns = val_columns

        with open(self.trn_csv_path, "w") as f:
            writer = csv.writer(f)
            writer.writerow(["step"] + trn_columns)
    ...
    def log_metrics(self, metrics, step):
        if "train_loss" in metrics:
            fields = [step] + [metrics[x] for x in self.trn_columns]
            filename = self.trn_csv_path
        else:
            epoch = metrics["epoch"]
            fields = [epoch] + [metrics[x] for x in self.val_columns]
